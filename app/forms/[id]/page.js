...
import ReactExportModal from "@/components/builder/ReactExportModal";
...
export default function FormBuilderPage() {
  ...
  const [reactExportOpen, setReactExportOpen] = useState(false);
  ...
  return (
    <ProtectedLayout>
        ...
        >
          ...
          <div className="flex shrink-0 items-center gap-3">
            ...
            <motion.button
              whileHover={{ scale: 1.03 }}
              whileTap={{ scale: 0.97 }}
              onClick={() => setReactExportOpen(true)}
              className="flex items-center gap-1.5 rounded-xl border border-indigo-200 bg-indigo-50 px-3 py-2.5 text-sm font-medium text-indigo-600 transition-all duration-200 hover:border-indigo-300 hover:bg-indigo-100 hover:text-indigo-700 dark:border-indigo-500/30 dark:bg-indigo-500/10 dark:text-indigo-400 dark:hover:border-indigo-500/50 dark:hover:bg-indigo-500/20 dark:hover:text-indigo-300"
            >
              <svg
                className="h-4 w-4"
                fill="none"
              ...
              <span className="hidden sm:inline">Export React</span>
            </motion.button>
      ...
      {/* React Component Export Modal */}
      <ReactExportModal
        isOpen={reactExportOpen}
        onClose={() => setReactExportOpen(false)}
        formName={formName}
      />
    </ProtectedLayout>
  );
